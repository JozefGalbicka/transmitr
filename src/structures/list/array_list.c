...
static void array_list_ensure_capcity(ArrayList *self) {
    if (self->size >= self->capacity) {
        self->capacity =
            (self->capacity > 0) ? 2 * self->capacity : DEFAULT_CAPACITY;
...
_Bool array_list_get_at(ArrayList *self, size_t index, void *item) {
    if (index < self->size) {
        memcpy(item, (unsigned char *)self->array + index * self->item_size,
               self->item_size);
        return true;
    } else {
        return false;
...
void array_list_add(ArrayList *self, const void *item) {
    array_list_ensure_capcity(self);
    memcpy((unsigned char *)self->array + self->size * self->item_size, item,
           self->item_size);
    self->size++;
}
...
_Bool array_list_insert_at(ArrayList *self, size_t index, const void *item) {
    if (index < self->size) {
        ...
        memmove((unsigned char *)self->array + (index + 1) * self->item_size,
                (unsigned char *)self->array + index * self->item_size,
                (self->size - index) * self->item_size);
        ++self->size;
        return true;
    } else if (index == self->size) {
        array_list_add(self, item);
        return true;
    } else {
        return false;
    }
...
_Bool array_list_set_at(ArrayList *self, size_t index, const void *item) {
    if (index < self->size) {
        memcpy((unsigned char *)self->array + index * self->item_size, item,
               self->item_size);
        return true;
    } else {
        return false;
...
void array_list_to_txt(ArrayList *self, FILE *f,
                       void (*item_to_file)(void *, FILE *)) {
    ArrayListIterator it;
    array_list_iterator_init(&it, self);
    while (array_list_iterator_has_next(&it)) {
        item_to_file(array_list_iterator_move_next(&it), f);
    }
    array_list_iterator_destroy(&it);
}
...
// Sorting operations
// ------------------
static void bubblesort(void *base, size_t nitems, size_t size,
...
void *array_list_iterator_move_next(ArrayListIterator *self) {
    void *tmp = (unsigned char *)self->list->array +
                self->index * self->list->item_size;
    self->index++;
    return tmp;
}
